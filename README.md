...
2. Replace **GRCh37-canbind-qc** with a name for the intermediate QC files generated by the pipeline. It is suggested to retain the -qc suffix to indicate that these are files used in the qc pipeline. 

Specific Snakefile rules require additional edits. 

#### Rule: HeterozygosityFilter 
- Change **/home/fosterlab/SA/GWAS/canbind/CBN_GWAS_files/GRCh37-run2** to the new directory containing the raw target data. This will need to be replaced for both the -f and -e flags.
